...
/**
 * Phase 1 — high-speed dogfight loop.
 */
export function createDogfightState(input) {
  ...
  function spawnWave() {
    ...
    for (let i = 0; i < ACTIVE_TIES; i++) {
      ...
      resetTie(t, 30 + i * ((100 - 30) / ACTIVE_TIES));
    }
  }

  return {
    enter(engine) {
      stars = new Starfield(140);
      player = new Player(engine.camera);
      ties = new Pool(createTie, TIE_POOL_SIZE);
      engine.camera.viewX = 0;
      engine.camera.viewY = 0;
      engine.camera.x = 0;
      engine.camera.y = 0;
      spawnWave();
    },
    ...
    update(engine, dt) {
      ...
      ties.updateEach(dt, (t) => updateTie(t, dt, engine.camera));

      while (ties.active.length < ACTIVE_TIES) {
        resetTie(ties.acquire(), 100 - Math.random() * 20);
      }
    },
